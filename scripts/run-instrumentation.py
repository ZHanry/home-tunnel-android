...
passed = (result.returncode == 0 and match is not None and int(match[1]) >= 15
          ...
          and "RemoteIdentityTest" in result.stdout
...
report = {"status": "passed" if passed else "failed", "api_level": int(api_level),
          ...
          "android_keystore": True, "administrator_ui": True, "administrator_identity": True,
          "remote_identity": True, "remote_media_backend": "unavailable", "variant": "debug"}
