...
def valid_contract_ref(value):
    """Pin stable or RC contracts without accepting branch names or loose SemVer."""
    number = r"(?:0|[1-9][0-9]*)"
...
if component == "server":
    ...
    assert valid_contract_ref(lock["ref"]), "Protocol source must use an exact stable or RC versioned ref"
    if component == "client":
